...
Now supports plugin system for multiple websites
"""
...
class AnimeDownloader:
    def __init__(self, config: Dict[str, Any] = None, plugin=None):
        # Use plugin if provided, otherwise fall back to AnimeKai logic
        self.plugin = plugin
        if plugin:
            self.BASE_URL = f"https://{plugin.domain}"
        else:
    ...
    def _find_executable(self, name: str) -> str:
        """Find executable in PATH or local directories"""
        ...
        if name == "ffmpeg":
            local_path = os.path.join(os.path.dirname(__file__), "..", "ffmpeg", "ffmpeg-8.0.1-essentials_build", "bin", "ffmpeg.exe")
            if os.path.exists(local_path):
                return local_path
        
        # For yt-dlp, use python -m yt_dlp
        if name == "yt-dlp":
    ...
    def get_anime_details(self, url: str) -> Tuple[Optional[str], str]:
        ...
        if self.plugin:
            return self.plugin.get_anime_details(url)

        # Original AnimeKai logic
        try:
    ...
    def get_episode_list(self, anime_id: str) -> List[Dict[str, Any]]:
        ...
        if self.plugin:
            return self.plugin.get_episode_list(anime_id)
    ...
    def get_video_servers(self, token: str) -> List[Dict[str, str]]:
        ...
        if self.plugin:
            return self.plugin.get_video_servers(token)
    ...
    def choose_server(self, servers: List[Dict[str, str]], prefer_type: str, prefer_server: str) -> Optional[Dict[str, str]]:
        ...
        if self.plugin:
            return self.plugin.choose_server(servers, prefer_type, prefer_server)
    ...
    def get_video_data(self, server_id: str) -> Optional[Dict[str, Any]]:
        ...
        if self.plugin:
            return self.plugin.get_video_data(server_id)
    ...
    def generate_episode_filename(self, anime_title: str, season_num: int, ep_id: str) -> str:
        ...
        if self.plugin:
            return self.plugin.generate_episode_filename(anime_title, season_num, ep_id)
    ...
    def download_with_ytdlp(self, url: str, output_file: str, episode_label: str, subtitles: List[Dict] = None, job=None) -> bool:
        ...
        try:
            ...
            # Start size monitoring thread if job is provided
            size_monitor_thread = None
            if job:
                def monitor_size():
                    import time
                    last_size = 0
                    while True:
                        ...
                        if current_size > last_size:
                            job.downloaded_size += (current_size - last_size)
                            last_size = current_size
                        
                        # Stop monitoring if final file exists and no .part file
                        if os.path.exists(output_file) and not os.path.exists(part_file):
                            break
                    
                import threading
                size_monitor_thread = threading.Thread(target=monitor_size, daemon=True)
            ...
            if subtitles:
                ...
                # Update size for temp video
                if job and os.path.exists(temp_video):
                    job.downloaded_size += os.path.getsize(temp_video)
                ...
                # Merge with ffmpeg
                if sub_files:
                    ffmpeg_cmd = [self.ffmpeg_path, "-i", temp_video]
    ...
    def download_episode(self, video_data: Dict[str, Any], output_file: str, episode_label: str, job=None) -> bool:
        ...
        for attempt in range(1, self.config["max_retries"] + 1):
            ...
            if self.download_with_ytdlp(url, output_file, episode_label, subtitles, job):
                self.log("INFO", f"[SUCCESS] Successfully downloaded episode {episode_label}")
                return True
    ...
    def merge_videos(self, file_list: List[str], anime_title: str, season_num: int, 
        ...
        try:
            ...
            self.log("INFO", f"[SUCCESS] Successfully merged: {merged_filename}")
            return merged_path
        except Exception as e:
