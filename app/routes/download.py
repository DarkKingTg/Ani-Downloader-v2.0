...
def run_download_job(job: DownloadJob, download_folder):
    ...
    try:
        ...
        # Log which plugin is being used
        plugin_name = plugin.name if plugin else "AnimeKai (default)"
        job.add_log("INFO", f"Using plugin: {plugin_name}")
        ...
        for idx, ep in enumerate(selected, 1):
            ...
            if downloader.download_episode(video_data, filepath, ep_id):
                ...
                job.add_log("INFO", f"[SUCCESS] Successfully downloaded episode {ep_id}")
            else:
                job.add_log("ERROR", f"[ERROR] Failed to download episode {ep_id}")
        ...
        if merge_episodes and len(downloaded_files) > 1:
            ...
            if merged_file:
                ...
                job.add_log("INFO", f"[SUCCESS] Successfully merged into {job.merged_file}")
                ...
                job.add_log("ERROR", "[ERROR] Merge failed")

        # Complete
        job.status = "completed"
        job.progress = 100
        job.end_time = datetime.now()
        job.add_log("INFO", f"[COMPLETED] Download job completed! Downloaded {job.completed_episodes}/{job.total_episodes} episodes")
